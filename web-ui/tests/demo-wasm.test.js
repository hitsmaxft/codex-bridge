...
test("turn summaries cover sparse message pages and hydrate exactly once", () => {
  const summary = {
      turn_id: "turn-history",
      message_index: 0,
      turn_stub: { start: 0, end: 500 },
    },
    tail = [
      {
        turn_id: "turn-active",
        message_index: 500,
        turn_stub: { start: 500, end: 992 },
      },
      { turn_id: "turn-active", message_index: 992 },
      { turn_id: "turn-active", message_index: 993 },
    ];
  assert.equal(messageRangesCoverPage([summary, ...tail], { start: 0, end: 994 }), true);
  assert.equal(messageRangesCoverPage([summary, tail[1]], { start: 0, end: 993 }), false);
  const hydrated = new Map([
      [0, { turn_id: "turn-history", message_index: 0 }],
      [1, { turn_id: "turn-history", message_index: 1 }],
    ]),
    merged = mergeHydratedTurnSequence([summary, ...tail], (turnId) =>
      turnId === "turn-history" ? hydrated : null,
    );
  assert.deepEqual(
...
test("large-session deferred turns hydrate only after explicit expansion", async () => {
  ...
  assert.match(source, /message\.turn_stub/);
  assert.match(source, /m\.turn_stub\.kind === "prefix"/);
  assert.match(stylesheet, /\.turn-group\.lazy\s*\{[^}]*content-visibility:\s*auto/s);
